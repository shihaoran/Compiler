...
int handle_op(char *op1,char *op2)//生成op所对应的机器码，将结果存在s0,s1寄存器中,-1都空,0只有op1,1只有op2,2都有
{
	...
	if (strcmp(op1,""))//如果op1不为空
	{
		...
		if (flag_reg)
			sprintf(op1, "s0");//如果不是分配寄存器的变量，存在s0中
	}
	if (strcmp(op2, ""))//如果op2不为空
	{
		...
		if (flag_reg)
			sprintf(op2, "s1");//如果不是分配寄存器的变量，存在s1中
	}
	if (flag1&&flag2)
		return 2;
	else if (flag1)
...
void gen_add()
{
	...
	if (handle_result(opr))//如果是寄存器，不用再存了
	{
...
void process_block(int start, int end)
{
	...
	for (i = start; i <= end; i++)
	{
		if(!optquat_table[i].is_empty)//空就不处理，但是要加四元式生成指针
...
int process_quat(struct quat_record *quat,int t)//t为2表示是全局常数四元式，1表示是局部常数四元式，0表示是其他
{
	...
	else//变量
	{
		...
		else if (isdigit(quat->op1[0]))//是立即数
		{
		...
		if (strcmp(quat->opr, ""))//存在opr
		{
			...
			else if (flag1 && (quat->op == NEG || quat->op == MOV))//处理NEG,MOV
			{
				if (quat->op == NEG)
				{
					char temp[MAX_OP_LEN]="";
...
void never_jmp()//用于死代码删除
{
	...
	for (i = 0; i < block_table[_func_ptr][n].prev_len; i++)
	{
		...
		{
			j = i;
			while (j < block_table[_func_ptr][n].prev_len - 1)//删除prev后前移
			{
				block_table[_func_ptr][n].prev[j] = block_table[_func_ptr][n].prev[j + 1];
				j++;
			}
			block_table[_func_ptr][n].prev_len--;//长度-1
		}
	}
	optquat_table[block_table[_func_ptr][n].start].label = -1;//删除跳转到的标签
}
