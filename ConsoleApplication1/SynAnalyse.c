...
int handle_op(char *op1, char *op2)//生成op所对应的机器码，将结果存在s0,s1寄存器中
								   //-1报错,0只有op1,1只有op2,2都有
{
	int offset = 0;
	int flag1 = 0;
	int flag2 = 0;
	if (strcmp(op1, ""))//如果op1不为空
	{
		...
		else if (op1[0] <= '9'&&op1[0] >= '0')//如果是立即数
		{
			offset = atoi(op1);
	...
	if (strcmp(op2, ""))//如果op2不为空
	{
		...
		else if (op2[0] <= '9'&&op2[0] >= '0')//如果是立即数
		{
			offset = atoi(op2);
